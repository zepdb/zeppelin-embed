...
for rep,order in enumerate([['before','after'],['after','before'],['before','after']],1):
 for arm in order:
  ...
  data=json.loads((destination/'results.json').read_text());assert len(data['cases'])==3
  for case in data['cases']:
   for field,n in [('warm_us',128),('setup_us',64),('present_us',64),('controls',64)]:assert len(case[field])==n
