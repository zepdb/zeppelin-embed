...
root=Path.cwd();out=root/'tasks/evidence/astra-16-measured'
def sha(p): return hashlib.sha256(Path(p).read_bytes()).hexdigest()
head=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip()
assert head.startswith('7d0f9ef'),head
scratch=Path(sys.argv[1]) if len(sys.argv)>1 else Path(tempfile.mkdtemp(prefix='ze-astra16-'))
...
example='crates/zeppelin-embed-bench/examples/lexical-assembly-cost.rs'
shutil.copy2(root/example,parent/example)
changed=subprocess.check_output(['git','diff','--name-only'],text=True).splitlines()
changed += ['crates/zeppelin-embed/src/lifecycle/lexical_assembly_tests.rs',example]
changed = sorted(set(changed))
(root/'tasks/evidence/astra-16-measured/source.patch').write_bytes(subprocess.check_output(['git','diff']))
...
receipts=[]
for arm,tree in [('before',parent),('after',root)]:
 package=scratch/arm;package.mkdir(exist_ok=True)
 manifest=f'''[package]
name = "lexical-assembly-screen"
version = "0.0.0"
edition = "2024"
[workspace]
[dependencies]
...
'''
 ...
 binary=out/('screen-'+arm);shutil.copy2(target/'release/lexical-assembly-screen',binary)
