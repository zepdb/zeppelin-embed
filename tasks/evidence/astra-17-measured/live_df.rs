...
mod tests {
    ...
    fn astra_17_live_df_eviction_and_large_keys_preserve_exact_values() {
        ...
        assert!(bytes > 0);
        for term in 0..256 {
            // Long keys force both entry-capacity and byte-capacity eviction.
            let key = format!("missing-{term:04}-{}", "z".repeat(term % 120));
            assert_eq!(
                cache
                    .frequency(&segment, &live, key.as_bytes(), &[FieldId(0)])
                    .unwrap(),
                0
            );
            assert_eq!(
