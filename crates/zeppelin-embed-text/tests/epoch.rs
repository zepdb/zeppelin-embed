...
#[test]
fn astra_08_old_tokenizer_epoch_is_refused_without_writes() {
    ...
    let error = match TextStore::open(&store_path, &bundle_path, Default::default()) {
        Ok(store) => {
            ...
            panic!("old tokenizer epoch must refuse open; vectors need re-embedding");
        }
        Err(error) => error,
    };
    assert!(matches!(
