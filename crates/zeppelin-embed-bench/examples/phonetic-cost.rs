//! Step 14: lazy phonetic expansion through real Store query paths.
...
fn query(id: usize, mode: &str) -> LexicalQuery {
    ...
    if mode == "prefix" {
        LexicalQuery::prefix(term.as_bytes().to_vec(), DEFAULT_FIELD)
    } else {
        LexicalQuery::phonetic(term.as_bytes().to_vec(), DEFAULT_FIELD)
    }
}
...
fn work() -> Value {
    ...
    json!({"builds":w.builds,"copied_bytes":w.copied_bytes,"terms_visited":w.terms_visited,"seek_steps":w.seek_steps,"encodings":observer::phonetic_encoding_calls()})
}
...
fn main() -> Result<(), Box<dyn Error>> {
    let mode = std::env::args()
        .nth(1)
        .ok_or("rare, collision, hybrid, or prefix required")?;
    if !matches!(mode.as_str(), "rare" | "collision" | "hybrid" | "prefix") {
        return Err("invalid mode".into());
    }
    let dir = tempfile::tempdir()?;
    let store = Store::open(dir.path(), OpenOptions::default())?;
    let documents = (0..4098)
        .map(|id| {
            IngestDocument::new(
                DocumentVersion::new(DocId::new(id as u128 + 1), Revision::new(1)),
                vec![1.0, id as f32 / 4098.0],
            )
            .with_text(match id {
                0 => "anchor night".to_owned(),
                1 => "anchor knight".to_owned(),
                _ => format!(
                    "anchor {}{}",
                    if id < 2050 { "smithson" } else { "robert" },
                    suffix(id)
                ),
            })
        })
    ...
    for id in 0..16 {
        store.ingest(IngestBatch::new(vec![
            ...
            .with_text(format!("anchor smithson{}", suffix(5000 + id))),
        ]))?;
        observer::begin();
    ...
    println!(
        "{}",
        json!({"mode":mode,"rows":4098,"dimensions":2,"warmups":20,"k":10,"cold":cold,"cold_work":cold_work,"warm_work":warm_work,"cache_before":cache_before,"cache_after":cache_after,"cache_final":cache_final,"steady":steady,"updates":updates})
    );
    Ok(())
}
