...
fn validate_fixture(
    ...
    let counters = ComparableCounters {
        ...
        bytes_read: first.stats.bytes_read,
...
fn write_counters(
    ...
    writeln!(
        output,
        "deterministic counters: scheme={:?} fixture={:?} shape={}x{} dims_touched={} bytes_read={} threads_used={}",
        config.scheme,
        config.fixture,
        config.rows,
        config.dimensions,
...
fn parse_args_from(arguments: impl IntoIterator<Item = String>) -> Result<Config, Box<dyn Error>> {
    ...
    if config.rows == 0 || config.dimensions == 0 || config.iterations == 0 {
        return Err("rows, dimensions, and iterations must be greater than zero".into());
    }
    Ok(config)
}
