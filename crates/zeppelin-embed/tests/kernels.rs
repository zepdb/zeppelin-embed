...
mod kernels {
    ...
    #[test]
    fn vertical_pdx_kernels_match_horizontal_oracles_for_all_block_tails() {
        assert_eq!(KERNEL_KNOB_SPACE.vertical_rows_per_tile, &[16, 32]);
        for rows in [1_usize, 3, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128] {
            for dimensions in [1_usize, 2, 3, 4, 7, 32, 33] {
                let query_f32 = (0..dimensions)
                    .map(|dimension| (dimension as f32 - 11.0) / 7.0)
                    .collect::<Vec<_>>();
                let row_major_f32 = (0..rows * dimensions)
                    .map(|index| ((index.wrapping_mul(29) % 101) as f32 - 50.0) / 13.0)
                    .collect::<Vec<_>>();
                let columns_f32 = pdx_f32_columns(&row_major_f32, rows, dimensions);

                let query_f16 = query_f32
                    .iter()
                    .map(|value| finite_f16_bits(*value))
                    .collect::<Vec<_>>();
                let row_major_f16 = row_major_f32
                    .iter()
                    .map(|value| finite_f16_bits(*value))
                    .collect::<Vec<_>>();
                let columns_f16 = pdx_f16_columns(&row_major_f16, rows, dimensions);

                let query_i8 = (0..dimensions)
                    .map(|dimension| (dimension.wrapping_mul(17) as i8).wrapping_sub(63))
                    .collect::<Vec<_>>();
                let row_major_i8 = (0..rows * dimensions)
                    .map(|index| (index.wrapping_mul(31) as i8).wrapping_add(7))
                    .collect::<Vec<_>>();
                let columns_i8 = pdx_i8_columns(&row_major_i8, rows, dimensions);

                let row_bytes = dimensions.div_ceil(2);
                let mut row_major_bit4 = (0..rows * row_bytes)
                    .map(|index| index.wrapping_mul(37) as u8)
                    .collect::<Vec<_>>();
                if !dimensions.is_multiple_of(2) {
                    for row in row_major_bit4.chunks_exact_mut(row_bytes) {
                        if let Some(last) = row.last_mut() {
                            *last &= 0xf0;
                        }
                    }
                }
                let columns_bit4 = pdx_bit4_columns(&row_major_bit4, rows, row_bytes);

                let scalar = KernelVariant::scalar();
                let expected_f32 = row_major_f32
                    .chunks_exact(dimensions)
                    .map(|row| scalar.dot_f32(&query_f32, row))
                    .collect::<Vec<_>>();
                let expected_f16 = row_major_f16
                    .chunks_exact(dimensions)
                    .map(|row| scalar.dot_f16(&query_f16, row))
                    .collect::<Vec<_>>();
                let expected_i8 = row_major_i8
                    .chunks_exact(dimensions)
                    .map(|row| scalar.dot_i8(&query_i8, row))
                    .collect::<Vec<_>>();
                let expected_bit4 = row_major_bit4
                    .chunks_exact(row_bytes)
                    .map(|row| scalar.dot_bit4(&query_i8, row))
                    .collect::<Vec<_>>();

                for variant in KernelVariant::available() {
                    let mut actual_f32 = vec![0.0_f32; rows];
                    variant.vertical_f32(&query_f32, &columns_f32, rows, &mut actual_f32);
                    let mut actual_f16 = vec![0.0_f32; rows];
                    variant.vertical_f16(&query_f16, &columns_f16, rows, &mut actual_f16);
                    let mut actual_i8 = vec![0_i32; rows];
                    variant.vertical_i8(&query_i8, &columns_i8, rows, &mut actual_i8);
                    let mut actual_bit4 = vec![0_i32; rows];
                    variant.vertical_bit4(&query_i8, &columns_bit4, rows, &mut actual_bit4);

                    for (row_id, (&expected, &actual)) in
                        expected_f32.iter().zip(&actual_f32).enumerate()
                    {
                        let row = &row_major_f32[row_id * dimensions..(row_id + 1) * dimensions];
                        assert_f32_result_matches(&query_f32, row, expected, actual);
                    }
                    for (row_id, (&expected, &actual)) in
                        expected_f16.iter().zip(&actual_f16).enumerate()
                    {
                        let row = &row_major_f16[row_id * dimensions..(row_id + 1) * dimensions];
                        assert_f16_result_matches(&query_f16, row, expected, actual);
                    }
                    assert_eq!(actual_i8, expected_i8, "arm={:?}", variant.arm());
                    assert_eq!(actual_bit4, expected_bit4, "arm={:?}", variant.arm());
                }
            }
        }
    }

    fn pdx_f32_columns(rows: &[f32], row_count: usize, dimensions: usize) -> Vec<u8> {
        let mut columns = Vec::with_capacity(std::mem::size_of_val(rows));
        for dimension in 0..dimensions {
            for row in 0..row_count {
                columns.extend_from_slice(&rows[row * dimensions + dimension].to_le_bytes());
            }
        }
        columns
    }

    fn pdx_f16_columns(rows: &[u16], row_count: usize, dimensions: usize) -> Vec<u8> {
        let mut columns = Vec::with_capacity(std::mem::size_of_val(rows));
        for dimension in 0..dimensions {
            for row in 0..row_count {
                columns.extend_from_slice(&rows[row * dimensions + dimension].to_le_bytes());
    ...
    fn pdx_i8_columns(rows: &[i8], row_count: usize, dimensions: usize) -> Vec<u8> {
        let mut columns = Vec::with_capacity(rows.len());
        for dimension in 0..dimensions {
            for row in 0..row_count {
                columns.push(rows[row * dimensions + dimension] as u8);
            }
        }
        columns
    }

    fn pdx_bit4_columns(rows: &[u8], row_count: usize, row_bytes: usize) -> Vec<u8> {
        let mut columns = Vec::with_capacity(rows.len());
        for column in 0..row_bytes {
            for row in 0..row_count {
                columns.push(rows[row * row_bytes + column]);
            }
        }
        columns
    }

    fn finite_f16_bits(value: f32) -> u16 {
        let sign = ((value.to_bits() >> 16) & 0x8000) as u16;
        let magnitude = value.abs().clamp(0.0, 65_504.0);
        if magnitude == 0.0 {
            return sign;
        }
        let exponent = magnitude.log2().floor() as i32;
        let half_exponent = (exponent + 15).clamp(1, 30) as u16;
        let base = 2.0_f32.powi(exponent);
        let fraction = (((magnitude / base - 1.0) * 1_024.0).round() as i32).clamp(0, 1_023);
        sign | (half_exponent << 10) | fraction as u16
    }
