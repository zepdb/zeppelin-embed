...
int main(int argc, char **argv) {
    /* The directory is the database. Pass a path to keep it after the run. */
    const char *path = argc > 1 ? argv[1] : "zeppelin-example-index";
    /* Applications supply document and query vectors from the same embedding
       space; this tiny example uses four dimensions for readability. */
    ...
    /* ABI structs are size-versioned, so initialize abi_size on each one. */
    ...
    /* Ingest the five documents as one visible generation. */
    ...
    /* Search for the three nearest documents. */
    ...
    /* Free callee-owned results, then close the persistent store handle. */
