...
def main() -> int:
    # Applications supply document vectors and stable document IDs. A later
    # write with a higher revision can replace an existing document.
    doc_ids = [doc_id for doc_id, _ in DOCUMENTS]
    vectors = np.asarray([vector for _, vector in DOCUMENTS], dtype=np.float32)

    # The store is a persistent directory. A temporary one keeps the example
    # self-cleaning; use an application path to reopen the data later.
    with (
        tempfile.TemporaryDirectory() as scratch,
        ze.open(Path(scratch) / "store") as store,
    ):
        # The complete batch becomes visible in the returned generation.
        ...
        # Query vectors must share the document vectors' dimension and
        # embedding space. Search returns the nearest three documents.
